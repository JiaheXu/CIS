...
def main():
    ...
    d_k = points_transform(F_reg , d_k) 
    ...
    output_dir = 'PA4-' + letters[num] + '-' + runtype[num] + '-Output.txt'
    ...
    if num < 6 and args.eval:
        ...
        c_eval = [np.round(np.average(c_diff), decimals=2), np.max(c_diff), np.min(c_diff), ((c_diff[c_diff > 0.1]).shape[0] / c_size)]
        d_eval = [np.round(np.average(d_diff), decimals=2), np.max(d_diff), np.min(d_diff), ((d_diff[d_diff > 0.1]).shape[0] / c_size)]
        dkck_eval = [np.round(np.average(dc_diff), decimals=2), np.max(dc_diff), np.min(dc_diff), ((dc_diff[dc_diff > 0.1]).shape[0] / N_samples)]
        ...
        dkck_eval.insert(0, "sk_ck")
        log_dir = "EVAL"
        if not (os.path.exists(os.path.join(os.getcwd(), log_dir))):
            os.mkdir(log_dir)
        logs_name = 'PA4-' + letters[num] + '-' + runtype[num] + '-Eval.txt'
