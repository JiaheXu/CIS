...
def optical_tracking( data_dir ,pa , data_type , letter  , output = 1 ):

################## read in data
    optpivot = glob.glob(data_dir + pa + data_type + '-' + letter + '-optpivot.txt')
    #print(optpivot)
...
####################seperate read in data
    ...
    calbody = glob.glob(data_dir + pa + data_type + '-' + letter + '-calbody.txt')
...
#################### end of seperate read in data


    #set an initial value, so we can use np.vstack
    F_d = np.zeros((1, 4, 4))
    # print(D_i)
    # print(d_i)
...
############### ans contains P_tip and P_dimple, p is marker positions in initial tool frame
    ans, p = pivot_calibration( data ,Nh, Nframes)
